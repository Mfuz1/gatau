//FULL FREE WEB APIKEY TAPI LU BELI SENDIRI LAH
//THANKS TO 
//MFUZ
//NURUL
//YOGI
...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Mfuzツ\n' //ganti nama lu! 
            + 'ORG: Owner Fuz-Bot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=62895619379748:+62 81953360563\n'  //ganti nomor lu! 
            + 'END:VCARD'
prefix = '#'
blocked = []   
limitawal = 10000
...
/******** OWNER NUMBER**********/
const ownerNumber = ["62895619379748@s.whatsapp.net"]   //ganti nomor lu! 
const pacarNumber = ["6281953360563@s.whatsapp.net"]  //ganti nomor lu!  
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('Subcribe Yt Mfuz, Scan now..','white'),color('Mfuzツ','red'),color('X','lime'),color('Yogi','red'))
})
	...
	client.on('message-new', async (mek) => {
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("JAGA UCAPAN DONG!! 😠")
                        ...
                        .then(() => {
                            client.sendMessage(from, `*「 ANTI BADWORD 」*\nFUZ-Bot akan kick kamu karena berkata kasar!`, text ,{quoted: mek})
		...
		setTimeout( () => {
			...
			switch(command) {
				...
				case 'addbadword':  // Fix Bug By FUZBot & Yogi*
                    ...
                    break
                case 'delbadword':  // Fix Bug By FUZBot & Yogi*
                    if (!isOwner) return reply(ind.ownerb())
                    if (!isGroupAdmins) return reply(ind.admin())
                    if (args.length < 1) return reply( `Kirim perintah ${prefix}addbadword [kata kasar]. contoh ${prefix}addbadword bego`)
                    ...
                    break 
                case 'listbadword':  // Fix Bug By FUZBot & Yogi*
                    ...
                    break 
				case 'getsticker':
				case 'gets':  // Fix Bug By FUZBot & Yogi*
					...
					break
				case 'stickerlist':
				case 'liststicker':  // Fix Bug By FUZBot & Yogi*
					...
					break
				case 'addsticker':  // Fix Bug By FUZBot & Yogi*
					...
					break
				case 'addvn':  // Fix Bug By FUZBot & Yogi*
					...
					break
				case 'getvn':  // Fix Bug By FUZBot & Yogi*
					...
					break
				case 'listvn':
				case 'vnlist':  // Fix Bug By FUZBot & Yogi*
					...
					break
				case 'addimage':  // Fix Bug By FUZBot & Yogi*
					...
					break
				case 'getimage':  // Fix Bug By FUZBot & Yogi*
					...
					break
				case 'imagelist':
				case 'listimage':  // Fix Bug By FUZBot & Yogi*
					...
					break
				case 'addvideo':  // Fix Bug By FUZBot & Yogi*
					...
					break
				case 'getvideo':  // Fix Bug By FUZBot & Yogi*
					...
					break
				case 'listvideo':
				case 'videolist':  // Fix Bug By FUZBot & Yogi*
		...
		case 'mutual':// Fix Case By Mfuz/Yogi/Hans⛔
				...
				break
		case 'next':// Fix Case By Mfuz/Yogi/Hans⛔
                 if (!isRegistered) return reply( ind.noregis())
				if (isGroup) return  reply( 'Command ini tidak bisa digunakan di dalam grup!')
				anug = getRegisteredRandomId(_registered).replace('@s.whatsapp.net','')
				await reply('Find for a partner...')
		...
		case 'transfer':
				...
				if (!q.includes('|')) return  reply(ind.wrongf())
                		...
                		addKoinUser('62895619379748@s.whatsapp.net', fee)
		...
		case 'giftlimit': 
				...
				const nomerr = args[0].replace('@','')
                		...
                		if (!nomerr) return reply(`maaf format salah\nmasukan parameter yang benar\ncontoh : ${prefix}giftlimit @62895619379748 20`)
		...
		case 'cuttly':
				...
				case 'pornhub':
					...
					var porn = gh.split("/")[0];
					var hub = gh.split("/")[1];
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}pornhub Mfuz/Hub`)
					reply(ind.wait())
				...
				case 'googletext':
					...
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}googletext Mfuz/gans/abiezz`)
					reply(ind.wait())
				...
				case 'phcomment':
					...
					var yogi = gh.split("/")[0];
					var cmnt = gh.split("/")[1];
					if (args.length < 1) return reply(`ã€Œâ—ã€Contoh : ${prefix}phcomment Mfuz/yahahaha`)
					reply(ind.wait())
				...
				case 'wallgravity':
					...
					reply(ind.wait())
					yogi = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome2/wallgravity?apikey=kalemhirupmah212&text1=${t1}&text2=${t2}`)
					client.sendMessage(from, yogi, image, {quoted: mek})
					await limitAdd(sender)
				break
				case 'space':
					...
					reply(ind.wait())
					yogi = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome2/space?apikey=kalemhirupmah212&text1=${t1}&text2=${t2}`)
					client.sendMessage(from, yogi, image, {quoted: mek})
					await limitAdd(sender)
				break
				case 'marvelstudio':                	
					...
					reply(ind.wait())
					yogi = await getBuffer(`http://lolhuman.herokuapp.com/api/textprome2/marvelstudio?apikey=kalemhirupmah212&text1=${t1}&text2=${t2}`)
					client.sendMessage(from, yogi, image, {quoted: mek})
					await limitAdd(sender)
				break
		case 'tinyurl':
		...
		case 'shrtco':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				client.updatePresence(from, Presence.composing) 
				data = await fetchJson(`http://lolhuman.herokuapp.com/api/shortlink2?url=${args[0]}&apikey=kalemhirupmah212`)
				hasil = `link : ${args[0]}\n\nOutput : ${data.result}`
				reply(hasil)
				await limitAdd(sender)
				break
		...
		case 'randomhentai':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                   gatauda = body.slice(13)
					reply(ind.wait())
                    anu = await fetchJson(`http://lolhuman.herokuapp.com/api/nhentai/344253?apikey=kalemhirupmah212`, {method: 'get'})
                    buffer = await getBuffer(anu.result.info.characters.tags.languages.image)
		...
		case 'galaxtext':
				...
				ct = body.slice(10)
				reply(ind.wait())
				ct = await getBuffer(`http://lolhuman.herokuapp.com/api/ephoto1/galaxywallpaper?apikey=kalemhirupmah212&text=${ct}`)
				client.sendMessage(from, ct, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		case 'bplogo':
		...
		case 'darkneon':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    ...
                    client.sendMessage(from, buffer, image, {caption: 'Nih kak.. *Jangan lupa follow ig @mfuz_*', quoted: mek})
         break
         case 'gplaybutton':
                case 'splaybutton':
                if (!isRegistered) return reply( ind.noregis())
...
//dappa gay
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  hehe = await getBuffer(`https://onlydevcity.herokuapp.com/api/maker/wasted?avatar=https://i.ibb.co/sCrqp32/b7512820a9d4.jpg&apikey=Fuzbot_3i`)
	...
	break
		...
		case 'fftourserti': 				
				...
				client.sendMessage(from, buffer, image, {quoted: mek, caption: '*Nih kak.. jangan lupa follow ig @mfuz_*'})
				break
		case 'fftourserti2': 				
				if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
		...
		case 'imgmaker':
				...
				ainez = body.slice(10)
				reply(ind.wait())
				anu = await fetchJson(`http://lolhuman.herokuapp.com/api/quotemaker?apikey=kalemhirupmah212&text=${ainez}`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {caption: 'Nih kak udah jadi..', quoted: mek})
				await limitAdd(sender)
				break
		...
		case 'shota':
				...
				reply(ind.wait())
				gatauda = body.slice(8)
				anu = await fetchJson (`https://onlydevcity.herokuapp.com/api/sfw/megumin?apikey=Fuzbot_3i`, {method: 'get'})
				buffer = awaitgetBuffer(anu.result)
				client.sendMessage(from, buffer, image, {quoted: mek})
				await limitAdd(sender)
				break
				case 'tolol': 
				...
				reply(ind.wait())
				buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/toloserti?apikey=kalemhirupmah212&name=${gatauda}`, {method: 'get'})
				client.sendMessage(from, buffer, image, {quoted: mek})
				await limitAdd(sender)
				break
				case 'emoji2img': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				reply(ind.wait())
				anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/meme?apikey=kalemhirupmah212`, {method: 'get'})
				buffer = await getBuffer(anu.result)
				client.sendMessage(from, buffer, image, {quoted: mek})
				await limitAdd(sender)
				break
				...
				gatauda = body.slice(8)
				reply(ind.wait())
				buffer = await getBuffer(`http://lolhuman.herokuapp.com/api/meme/darkjoke?apikey=kalemhirupmah212`, {method: 'get'})
				client.sendMessage(from, buffer, image, {quoted: mek})
				await limitAdd(sender)
				break
		case 'randomkpop':
		...
		case 'vinta':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					...
					await limitAdd(sender)
					break	
                case 'faktaunik': // Fixed By Yogiツ & Mfuz
					// Fix Case By Mfuz/Yogi/Hans⛔
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/faktaunik?apikey=kalemhirupmah212`, {method: 'get'})
					reply(anu.result)
					await limitAdd(sender)
					break		
		case 'quotes':
		...
		case 'tampar':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					client.updatePresence(from, Presence.composing) 
					data = await fetchJson(`http://lolhuman.herokuapp.com/api/newsinfo?apikey=kalemhirupmah212`, {method: 'get'})
					teks = '=================\n'
					for (let i of data.result) {
		...
		case 'pinterest':
           ...
           case 'waifu':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://onlydevcity.herokuapp.com/api/nsfw/waifu?apikey=Fuzbot_3i`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           ...
           case 'blackpink':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					gatauda = body.slice(10)
					anu = await fetchJson(`https://onlydevcity.herokuapp.com/api/randomimage/blackpink?apikey=Fuzbot_3i`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek })
					await limitAdd(sender) 
					break
           ...
           case 'imagegame':
		     ...
		     case 'fakta':
				...
				case 'katabijak':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
					anu = await fetchJson(`https://onlydevcity.herokuapp.com/api/katabijak?apikey=Fuzbot_3i`, {method: 'get'})
		...
		case 'spamcall':
			...
			break  
               ...
               case 'ytmp3':
                ...
                case 'ytmp4':
                    ini_link = args[0]
                    get_result = await fetchJson(`https://api.xteam.xyz/dl/ytmp4?url=https://www.youtube.com/watch?v=XxAT-T3lk8E&APIKEY=FuzBot1&url=${ini_link}`)
                    get_result = get_result.result
                    txt = `Title : ${get_result.title}\n`
		...
		case 'cerpen': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/cerpen?apikey=kalemhirupmah212`, {method: 'get'})
		...
		case 'quotes': 
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/random/quotes?apikey=kalemhirupmah212`, {method: 'get'})
	  ...
	  case 'ig':
		...
		case 'infofilm2': 
                 if (!isRegistered) return reply( ind.noregis())
					...
					if (isBanned) return reply('Maaf kamu sudah terbenned!')
					ige = body.slice(10)
					anu = await fetchJson(`http://lolhuman.herokuapp.com/api/lk21?apikey=kalemhirupmah212&query=${ige}`)
					buffer7 = await getBuffer(anu.result.thumbnail)
		...
		case 'heroml':  
				...
				if (!isUrl(args[0]) && !args[0].includes('www.instagram.com')) return reply(mess.error.lv)
					 ige = body.slice(9)
                     anu = await fetchJson(`http://lolhuman.herokuapp.com/api/instagram?apikey=kalemhirupmah212&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
				...
				if (!isUrl(args[0]) && !args[0].includes('tiktok.com')) return reply(mess.error.lv)
					 ige = body.slice(12)
                     anu = await fetchJson(`http://lolhuman.herokuapp.com/api/tiktok?apikey=kalemhirupmah212&url=${args[0]}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
		...
		case 'grouplist':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group FUZ-BOT :\n\n\`\`\``
					no = 0
		...
		case 'hobby':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
					hobby = body.slice(1)
					const hob =['comli','Membantu Atok','Mabar','Nobar','Sosmedtan','Membantu Orang lain','Nonton Anime','Nonton Drakor','Naik Motor','Nyanyi','Menari','Bertumbuk','Menggambar','Foto fotoan Ga jelas','Maen Game','Berbicara Sendiri']
		...
		case 'menu': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    ...
                    mfuz = `◪ INFO
  ❏ Nama Owner: Mfuz
  ❏ Wa: wa.me/62895619379748
  ❏ Ig: https://instagram.com/mfuz_
  ❏ My name: Fuz - Bot
  ❏ My ig: https://instagram.com/fuzbot1_
  ❏ Prefix: 「  ${prefix}  」
  ❏ Register : *${_registered.length}*
  -----------------------------------
_Salken kak *${pushname}* Aku Robot whatsapp_
------------------------------------
◪ PROFILE KAMU :
  ❏ Nama: ${pushname}
  ❏ Duit: *Rp.${uangku}.-*
...
*${prefix}snk*
Untuk mengetahui Syarat & Ketentuan Bot
*${prefix}donasi*
Info untuk yang mau donasi
*${prefix}sewabot*
Info untuk yang mau sewa Bot 
Murah lohh
...
•Arul 818` 
                    ...
                    client.sendMessage(from, wew, image, { quoted: mek, caption: mfuz })
					break
					case 'soundmenu':
					if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
...
break 
case 'djwelot':
const djwelot = fs.readFileSync('mp3/djwelot.mp3')
client.sendMessage(from, djwelot, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sad':
...
break
case 'bot':
bot = fs.readFileSync('./mp3/bot.mp3');
client.sendMessage(from, bot, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'fuz':
fuz = fs.readFileSync('./mp3/fuz.mp3');
client.sendMessage(from, fuz, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'abangjago':
abangjago = fs.readFileSync('./mp3/abangjago.mp3');
client.sendMessage(from, abangjago, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'tapiboong':
tapiboong = fs.readFileSync('./mp3/tapiboong.mp3');
client.sendMessage(from, tapiboong, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'pota':
case 'potapota':
pota = fs.readFileSync('./mp3/pota.mp3');
...
break
		...
		case 'crosslogo':
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                    ...
                    client.sendMessage(from, tod, image, {caption: 'Nih kak.. *Jangan lupa follow ig @mfuz_*', quoted: mek})
         break
...
case 'bneon':
		...
		case 'lpr':
					...
					client.sendMessage('62895619379748@s.whatsapp.net', options, text, {quoted: mek})
					reply('Masalah telah di laporkan ke owner FUZ-BOT, Laporan palsu atau main² tidak akan ditanggapi.')
					break
		case 'request':
					...
					client.sendMessage('62895619379748@s.whatsapp.net', options, text, {quoted: mek})
					client.sendMessage('62895619379748@s.whatsapp.net', options, text, {quoted: mek})
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER FUZ-BOT, Requests palsu atau main² tidak akan ditanggapi.')
					break
		case 'blocklist': 
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
                              ...
                              cliend.sendMessage(from, tag, text, {quoted: mek})
					                           await limitAdd(sender)
                                        break
                                        case 'profile':
		...
		case 'hargaprem':
		case 'hargapremium':
					client.sendMessage(from, 'Level 100 = Free 3 day (premium)\n10k = 30day (premium) atau 20k (permanen)',MessageType.text, { quoted: mek} )
					break
		case 'buypremium':
					client.sendMessage(from, 'Ingin membeli premium?\nHarap hubungi owner : wa.me/628956193797478',MessageType.text, { quoted: mek} )
					break
		...
		case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST FUZ-BOT 」*\n\n${body.slice(4)}`)
						}
						reply('*Suksess broadcast* ')
					}
					break
		...
		case 'toimg':
				...
				if (!isQuotedSticker) return reply('*Reply/Tag sticker!*')
                 ...
                 case 'simi':
					...
					teks = body.slice(5)
					anu = await simih(teks) //fetchJson(`https://api.xteam.xyz/simsimi?&APIKEY=FuzBot1&text=${teks}`, {method: 'get'})
		...
		case 'clone':
            ...
            if (budy.includes(`assalamualaikum`)) {
                  ...
                  reply(`Waalaikumsalam kak kak ${pushname}`)
       ...
       if (budy.includes(`@6282129599612`)) {

                  reply(`Iyah Apa Ada Tag akoh? 🗣`)

                  }
                 
       if (budy.includes(`@62895619379748`)) {

                  reply(`Ya, Itu Owner Gua Dia mungkin Lagi Sibuk 🗣`)

                  }


       if (budy.includes(`Mfuz`)) {

                  reply(`Jangan Panggil Mfuz Kak, Dia Lagi Sibuk 🗣`)
       ...
       if (budy.includes(`mfuz`)) {
		...
		if (budy.includes(`Sayang`)) {
                  ...
                  reply(`Aku Juga Sayang Kamu Kak ${pushname}😍`)
       ...
       if (budy.includes(`Tq`)) {
        ...
        if (budy.includes(`Sad`)) {
                  ...
                  reply(`_*kasian padahal masi muda*_`)
       ...
       if (budy.includes(`Anj`)) {
                  ...
                  reply(`_*JANGAN TOXIC BRO DOSA*_`)
       ...
       if (budy.includes(`P`)) {
                  ...
                  reply(`_*Waalaikumsallam P mulu ketik #menu y*_`)
       ...
       if (budy.includes(`https://chat.whatsapp.com/`)) {
                  ...
                  reply(`_*Sewa bot bayar om ketik #sewabot*_`)

                  }
                  
                  
                  
      if (budy.includes(`Undangan`)) {
